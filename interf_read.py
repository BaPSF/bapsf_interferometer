...
Read port 20/29 phase (rad) and LeCroy time (ms) from the daily interferometer_data_YYYY-MM-DD.hdf5
files (schema: interf_file.init_hdf5_file). phase_p40/time_array_p40 are not read.

Times are fixed UTC-8 (PST), not DST-aware: during PDT, wall-clock times are one hour ahead of
what these functions print and expect.
'''
import os
import h5py
import numpy as np
...
def get_interf_data(year, month, day, hour, minute, second, data_path):
	"""The shot nearest the given UTC-8 time, within ±10 s: (t_ms, phase_p20, phase_p29, time string);
	four Nones if there is no file or no shot in range."""
...
def get_evenly_spaced_interf_data(year, month, day, data_path, n_traces=4):
	"""Up to n_traces shots with all of phase_p20/p29/time_array, evenly spaced through the day's
	file, as get_interf_data tuples; [] if none."""
	pacific = datetime.timezone(datetime.timedelta(hours=-8))  # PST (UTC-8)
	date_str = f"{year:04d}-{month:02d}-{day:02d}"
	filename = f"interferometer_data_{date_str}.hdf5"
	file_path = os.path.join(data_path, filename)
...
if __name__ == "__main__":
	...
	cal_20 = get_calibration_factor(288e9)
	cal_29 = get_calibration_factor(282e9)
